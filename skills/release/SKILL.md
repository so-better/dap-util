---
name: release
description: dap-util 发布流程，按步骤完成版本号更新、源码打包、文档更新、changelog 更新、文档打包、README 更新、npm 发布，每步均需用户确认后执行。
---
...
逐一读取以下源码文件，提取准确的 API 信息：

- `src/color.ts`：颜色相关工具方法的真实签名
- `src/common.ts`：通用工具方法的真实签名
- `src/data.ts`：数据处理相关方法的真实签名
- `src/element.ts`：DOM 元素相关方法的真实签名
- `src/event.ts`：事件相关方法的真实签名
- `src/file.ts`：文件相关方法的真实签名
- `src/number.ts`：数字处理相关方法的真实签名
- `src/platform.ts`：平台判断相关方法的真实签名
- `src/speech.ts`：语音相关方法的真实签名
- `src/string.ts`：字符串处理相关方法的真实签名
...
- `docs/guide/modules/color.md`：颜色模块文档
- `docs/guide/modules/common.md`：通用模块文档
- `docs/guide/modules/data.md`：数据模块文档
- `docs/guide/modules/element.md`：元素模块文档
- `docs/guide/modules/event.md`：事件模块文档
- `docs/guide/modules/file.md`：文件模块文档
- `docs/guide/modules/number.md`：数字模块文档
- `docs/guide/modules/platform.md`：平台模块文档
- `docs/guide/modules/speech.md`：语音模块文档
- `docs/guide/modules/string.md`：字符串模块文档
- `docs/guide/quick-start.md`：快速上手示例
- `docs/guide/introduction.md`：介绍页
...
用户确认后，**重新读取** `src/index.ts`、各模块源码文件以及更新后的 `docs/` 文档，对 `README.md` 进行核查和修正：
...
2. **简介**：根据 `docs/guide/introduction.md` 提炼，说明这是什么、解决什么问题
...
5. **快速上手**：包含基本使用示例，取自 `docs/guide/quick-start.md`
6. **模块列表**：列出所有模块（color、common、data、element、event、file、number、platform、speech、string）及简要说明，每个模块附带最小可用代码示例（取自对应文档）
7. **文档链接**：完整文档地址
8. **License**
...
> 即将执行 `npm publish` 将 `dap-util@{新版本号}` 发布到 npm，是否继续？
