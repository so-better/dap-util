...
module.exports = {
		...
		scrollEle.addEventListener('scroll', e => {
			if (this.getScrollTop(scrollEle) <= 0) { //滑动到顶部
				...
				if (scrollEle == window) {
					...
					height = numberUtil.subtract(scrollEle.offsetHeight, borderTop, borderBottom)
				}
				if ((numberUtil.add(this.getScrollTop(scrollEle), height) >= this.getScrollHeight(
						scrollEle)) && (height !=
	...
	getScrollWidth(el) {
		...
		let scrollWidth = 0
		if (this.isElement(el) && el != document.documentElement && el != document.body && el != window) {
			...
			if (document.documentElement.scrollWidth == 0 || document.body.scrollWidth == 0) {
				...
				scrollWidth = document.documentElement.scrollWidth > document.body.scrollWidth ? document
					.documentElement.scrollWidth : document.body.scrollWidth
			}
		}
		return scrollWidth
	},
	...
	setScrollTop(options) {
		...
		return new Promise((reslove, reject) => {
			if (time <= 0) {
				...
				let everTop = numberUtil.divide(numberUtil.subtract(number, nowTop),
					spacingIndex) // 计算每次滑动的距离
				let scrollTimer = setInterval(() => {
					if (spacingIndex > 0) {
						spacingIndex--
						if (isWindow) {
							document.documentElement.scrollTop = document.body.scrollTop = nowTop =
								numberUtil.add(nowTop, everTop)
						} else {
	...
	setScrollLeft(options) {
		...
		return new Promise((reslove, reject) => {
			if (time <= 0) {
				...
				let everLeft = numberUtil.divide(numberUtil.subtract(number, nowLeft),
					spacingIndex) // 计算每次滑动的距离
				let scrollTimer = setInterval(() => {
					if (spacingIndex > 0) {
						spacingIndex--
						if (isWindow) {
							document.documentElement.scrollLeft = document.body.scrollLeft =
								nowLeft = numberUtil.add(nowLeft, everLeft)
						} else {
	...
	getElementBounding(el) {
		...
		let bottom = numberUtil.subtract(document.documentElement.clientHeight || window.innerHeight, point
			.bottom) //元素底部距离可视窗口底部的距离
		let left = point.left //元素左侧距离可视窗口左边的距离
		let right = numberUtil.subtract(document.documentElement.clientWidth || window.innerWidth, point
			.right) //元素右侧距离可视窗口右边的距离
		return {
			top,
			bottom,
			left,
