    ...
    start(text, params) {
        return new Promise((resolve, reject) => {
            if (!window.SpeechSynthesisUtterance || !window.speechSynthesis) {
                throw new Error(
                    'The current browser does not support this syntax'
                )
            }
            let defaultParams = {
            ...
            speech.onstart = event => {
                defaultParams.start.apply(speech, [event, defaultParams])
            }
            //播放结束后调用
            speech.onend = event => {
                defaultParams.end.apply(speech, [event, defaultParams])
            }
            //播放暂停后调用
            speech.onpause = event => {
                defaultParams.pause.apply(speech, [event, defaultParams])
            }
            //播放恢复后调用
            speech.onresume = event => {
                defaultParams.resume.apply(speech, [event, defaultParams])
            }
            //播放出错后调用
            speech.onerror = event => {
                defaultParams.error.apply(speech, [event, defaultParams])
            }
            //加入播放队列
