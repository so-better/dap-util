...
- 示例
  ...
  ```ts
  import { file } from 'dap-util'
  const image = file.dataBase64toFile(base64String, 'image.png')
  ```
...
- 详细信息
  ...
  - `minSize`：类型为 `number`，表示原图的最小尺寸阈值，小于该大小的图片不进行压缩，单位 `kb`，默认为 0 表示任何图片都要压缩

- 示例

  ```ts
  import { file } from 'dap-util'
  const newImage = await file.compressImage(image, {
    width: 800,
    quality: 0.5
  })
  ```
