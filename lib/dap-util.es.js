...
const element = {
  ...
  rem2px(num) {
    ...
    return number.multiply(num, parseFloat(fs));
  },
  /**
   * rem与px单位转换
  ...
  scrollTopBottomTrigger(el, callback) {
    ...
    scrollEle.addEventListener("scroll", () => {
      if (this.getScrollTop(scrollEle) <= 0) {
        ...
        const scrollHeight = this.getScrollHeight(scrollEle === window ? void 0 : scrollEle);
        if (number.add(this.getScrollTop(scrollEle), height) + 1 >= scrollHeight && height != scrollHeight) {
          if (!flag) {
            return;
          }
          if (typeof callback == "function") {
  ...
  compressImage(file2, options) {
    const createFile = (canvas, fileName, quality) => {
      ...
      if (options.maxSize && options.maxSize > 0 && file3.size > options.maxSize * 1024) {
        if (quality <= 0) {
          return { file: file3, url, quality: 0 };
        }
